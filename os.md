...
一个进程可以分成多个**线程**，这些线程拥有不同的PC、不同的内存栈等等，唯一相同的是它们共享内存地址空间，即它们只有一套数据（特别地，所有线程共用一个堆，称为共享内存）。如果几个线程在同一个处理器上运行，线程之间也需要像进程之间一样不停切换，只是切换时不需要切换原有线程的地址空间了。线程的寄存器等也要被操作系统管理，存储这些数据的结构叫线程控制单元(TCB)。同一个进程中的多个线程分享堆，但不分享栈——这就意味着某些线程的栈空间会与其他线程的栈空间冲突，但在程序不使用大量递归时，这种线程处理方式一般并无大碍。
...
实现事务原子化的基础机制是**锁**，它是一个由硬件和操作系统提供给软件的接口，事实上就是一个变量。当某线程第一个请求某锁时，它将拥有这把锁，然后执行被锁住的代码。其他线程若想获得*这把锁*（可以同时有很多锁存在，因为一把锁就是一个变量），执行被*这把锁*锁住的代码，则必须等待第一个线程退出。当该线程退出时，所有等待线程中的一个将会被通知，它将获得这把锁，从而可以执行它被锁住的代码。通过锁可以保证关键代码的原子性（异步执行），从而在操作系统混乱不可预知的调度下保证关键代码的执行有序。

以上所谈论的锁都是互斥锁（mutex），它是其他锁的基础。实现互斥锁的关键之处在于实现互斥执行。在单处理器机器上，最简单的互斥执行方式是暂时禁止所有对该线程的中断，但很不幸的是，如果给予用户线程这样的权力，坏线程会利用这点运行恶意代码（如无穷循环），从而导致系统崩溃，因此这种锁只在操作系统内部有少量的使用。以上的解决方案也无法在多处理器机器上实现。第一个有效的锁是Test & Set自旋锁，即在尝试占有锁时，通过硬件支持把test & set变成一个不可拆分的原子表达式，这样就不会在线程运行到关键时刻时有其他线程被调度进来。自旋锁的“自旋”指的是若此锁正在被别的线程占据则本线程做死循环。自旋锁不是公平的锁，在单机上非常浪费，但在分布式系统上如果关键时刻做的操作不多，性能还不错。与之类似的算法还有compare & swap, load linked & store conditional, fetch & add等等（代码可以参考附录），这些都是自旋锁。

硬件对锁的支持 | 原子化操作描述
-|-
Test & Set | 尝试设置flag为1，返回flag原值
Compare & Swap | 比较flag，如果跟期待值相同则设置其为新值，否则不做任何操作，返回flag原值
Load Linked & Store Conditional | Load Linked开始监控对ptr值的更新，Store Conditional在ptr没有被更新过的前提下更新成功，否则更新失败
Fetch & Add | 返回flag原值，并把flag加1
