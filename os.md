...
事件并行的最大问题在于，任何系统调用都可能阻塞事件的处理。因此在handler中需要禁止任何导致中断的系统调用，比如输入输出等。为了解决事件并行要求输入输出的问题，现代系统中都提供了*异步输入输出*的功能。我们在3110中已经学过的promise，就是一种异步输入输出的实现机制。
...
* COW设计。不急着更改文件内容，而是把新内容写到永久存储中的空闲地区，过一段时间再把新内容更新。这就是LFS文件系统的主要思想。
